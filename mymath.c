...
/*---------------------------------*/
double LDirichletProb(const double prior[],const double post[],int length)
...
Log Mean = logmax -log(rep) + log(sum).  */
{
  ...
  return *logmax - log1p(rep) + log(*sum);
}
/*----------------------------------*/
...
/*------------------------------------------------------------*/
double ChiSq(const int *list1,int len1,const int *list2,int len2,int mincount,int missing,int *df)
